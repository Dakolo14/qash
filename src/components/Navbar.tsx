    ...
    <nav ref={navbarRef} className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-[#4b0d18] shadow-lg' : 'bg-transparent'}`}>
      <div className="container mx-auto flex justify-between items-center p-4">
        ...
        <div className="hidden md:!flex items-center text-white space-x-8">
          ...
          <Link href="/advisory" className="text-white hover:text-gray-400 transition-colors">Advisory</Link>
          <Link href="/apply" className="text-white hover:text-gray-400 transition-colors">Funding</Link>
          <Link href="/contact" className="text-white hover:text-gray-400 transition-colors">Contact</Link>
          <Link href="/contact" className={`${pathname === '/' ? 'bg-gradient-to-r from-orange-600 to-red-800 text-white hover:from-orange-700 hover:to-red-900' : 'bg-white text-[#4b0d18] hover:bg-gray-100'} font-semibold py-2 px-6 rounded-full transition-colors`}>
            Work With Us
          </Link>
        </div>
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-[#4b0d18] absolute top-full left-0 right-0 shadow-lg">
          <div className="flex flex-col items-center space-y-6 py-8">
            ...
            <Link href="/advisory" className="text-white text-lg hover:text-gray-200 transition-colors" onClick={toggleMenu}>
              Advisory
            </Link>
            <Link href="/apply" className="text-white text-lg hover:text-gray-200 transition-colors" onClick={toggleMenu}>
              Funding
            </Link>
            <Link href="/contact" className="text-white text-lg hover:text-gray-200 transition-colors" onClick={toggleMenu}>
              Contact
            </Link>
            <Link href="/contact" className={`${pathname === '/' ? 'bg-gradient-to-r from-orange-600 to-red-800 text-white hover:from-orange-700 hover:to-red-900' : 'bg-white text-[#4b0d18] hover:bg-gray-100'} font-semibold py-2 px-6 rounded-full transition-colors w-3/4`} onClick={toggleMenu}>
              Work With Us
            </Link>
          </div>
        </div>
